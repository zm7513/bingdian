...
<template>
  <div class="search">
            ...
            <Table border :columns="columns" :data="data" ref="table">
              ...
              <template slot-scope="{ row }" slot="validate">
                <i-switch
                  ...
                  :disabled="!row.editable||row.type=='upload'"
                >
                  <span slot="open">开</span>
                  <span slot="close">关</span>
                </i-switch>
    ...
    <Modal :title="modalTitle" v-model="modalVisible" :mask-closable="false" :width="750">
      <Form ref="form" :model="form" :label-width="85" :rules="formValidate" inline>
        <FormItem label="排序值" prop="sortOrder" style="width:340px">
          <InputNumber v-model="form.sortOrder"></InputNumber>
        </FormItem>
        <FormItem label="字段英文名" prop="field">
          <Tooltip placement="right" content="与后台实体字段匹配">
            <Input v-model="form.field" style="width:250px"/>
          </Tooltip>
        </FormItem>
        <FormItem label="名称" prop="name">
          <Input v-model="form.name" style="width:250px"/>
        </FormItem>
        <FormItem label="表格中显示" prop="tableShow" style="width:340px">
          <i-switch v-model="form.tableShow">
            <span slot="open">开</span>
            <span slot="close">关</span>
          </i-switch>
        </FormItem>
        <FormItem label="可添加编辑" prop="editable" style="width:340px">
        ...
        <FormItem label="显示级别" prop="level" v-show="form.type=='area'">
          <Select v-model="form.level" style="width:250px">
        ...
        <FormItem
          label="表单非空验证"
          prop="validate"
          v-show="form.editable&&form.type!='upload'"
          style="width:340px"
        >
        ...
        <FormItem label="可搜索" prop="searchable" style="width:340px">
        ...
        <FormItem label="搜索框类型" prop="searchType" v-show="form.searchable">
          <Select v-model="form.searchType" style="width:250px">
            <Option value="text">文本输入框</Option>
            <Option value="select">选择下拉框</Option>
            <Option value="daterange">日期范围选择器</Option>
            <Option value="date">日期选择器</Option>
        ...
        <FormItem label="可排序" prop="sortable" style="width:340px">
          <i-switch v-model="form.sortable">
            <span slot="open">开</span>
            <span slot="close">关</span>
          </i-switch>
        </FormItem>
        <FormItem label="默认排序" prop="defaultSort" v-show="form.sortable" style="width:340px">
        ...
        <FormItem label="默认排序方式" prop="defaultSortType" v-show="form.defaultSort&&form.sortable">
          <Select v-model="form.defaultSortType" style="width:250px">
            <Option value="desc">倒序(大到小)</Option>
            <Option value="asc">正序(小到大)</Option>
          </Select>
        </FormItem>
...
<script>
import { generateTable } from "@/api/index";
export default {
