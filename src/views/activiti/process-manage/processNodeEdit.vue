              ...
              >
                ...
                <div v-show="form.type==1">
                  ...
                  <div v-show="chooseUser">
                    <FormItem label="选择用户">
                      <user-choose buttonText="选择用户" @on-change="handleSelectUser" ref="user"></user-choose>
                    </FormItem>
                  </div>
                </div>
                <Form-item class="br">
    ...
    selectTree(v) {
      if (v) {
        ...
        if (data.users && data.users.length > 0) {
          ...
          this.$refs.user.setSelectUser(data.users);
        } else {
          this.chooseUser = false;
          this.$refs.user.setSelectUser([]);
        }
        // 回显角色
        ...
        if (data.departments && data.departments.length > 0) {
          ...
          this.$refs.depTree.setSelectDep(departmentIds, title);
          data.departmentIds = departmentIds;
        } else {
          this.chooseDepartment = false;
          this.$refs.depTree.setSelectDep([], "");
        }
        this.form = data;
      } else {
        this.cancelEdit();
